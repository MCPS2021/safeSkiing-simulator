...
class SafeSkiingSimulator:
        ...
        if slopes_conf_validator(c_slopes):
            for c_slope in c_slopes:
                ...
                if c_slope['name'] == 'root':
                    ...
                    self.slopes.append(Slope('root', slope_time=0))
                    continue

                print ("Adding slope {}".format(c_slope['name']))
                self.slopes.append(Slope(c_slope['name'], c_slope['slope_time']))
            ...
            for slope in self.slopes:
                ...
                #for every exit
                for exit in exits:
                    print ("Retrieving object of slope {}".format(exit['name']))
                    #create an array of dict, getting the slope object by name and the distribution
                    result.append({'object': get_slope_by_name(self.slopes, exit['name']), 'distribution':exit['distribution']})
                #set the exits to the slope
                print("Inserting exits to slope {}: {}".format(slope.name, result))
    ...
    def simulate(self, n_steps=1, sleep_time=1):
    ...
    def draw(self, stdscr):
        for i,slope in enumerate(self.slopes):
            stdscr.addstr(i,0,"Slope {}, Slope Queue: {}, Ski Lift Queue: {}".format(slope.name, len(slope.slope_queue), len(slope.ski_lift_queue)))
